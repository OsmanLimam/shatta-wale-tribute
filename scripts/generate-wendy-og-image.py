#!/usr/bin/env python3
"""
Generate a 1200x630 JPG OG image for the Wendy Shay Living Legacy site.

Design language mirrors the editorial site:
- Deep black background (#0a0a0a)
- Rose accent (#e0508c) — queen-of-afrobeats identity
- Ivory typography (#f4efe6)
...
HERO_PHOTO = "/home/z/my-project/public/images/wendy-shay/hero-stage.jpg"

# Editorial palette — Wendy Shay rose
INK = (10, 10, 10)
ROSE = (224, 80, 140)        # primary accent
ROSE_DIM = (163, 45, 99)     # darker rose
ROSE_SOFT = (240, 168, 197)  # lighter rose
IVORY = (244, 239, 230)
IVORY_FADE = (205, 198, 184)
...
def main():
    ...
    font_tagline = load_font(serif_italic, 30)
    font_meta = load_font(mono_path, 14)

    # --- Layout ---
    left_margin = 80

    # Eyebrow (top-left, rose, mono-caps style)
    eyebrow_y = 70
    eyebrow_text = "A LIVING LEGACY EXPERIENCE"
    draw.text((left_margin, eyebrow_y), eyebrow_text, fill=ROSE, font=font_eyebrow)

    # Rose accent line under eyebrow
    draw.line([(left_margin, eyebrow_y + 28), (left_margin + 80, eyebrow_y + 28)], fill=ROSE, width=2)

    # Main name (huge serif) — "WENDY" ivory, "SHAY" rose
    name_y = 180
    draw.text((left_margin, name_y), "WENDY", fill=IVORY, font=font_name)
    draw.text((left_margin, name_y + 130), "SHAY", fill=ROSE, font=font_name)

    # Tagline (italic serif, beneath name)
    tagline_y = name_y + 290
    draw.text(
        (left_margin, tagline_y),
    ...
    meta_y = H - 60
    draw.text((left_margin, meta_y), "AN INDEPENDENT DIGITAL ARCHIVE", fill=ROSE_DIM, font=font_meta)
    draw.text(
        (left_margin, meta_y + 22),
        "CREATED WHILE THE STORY IS STILL BEING WRITTEN",
        fill=IVORY_FADE,
        font=font_meta,
    )

    # Right side: rose accent diamond
    cx, cy = W - 100, H // 2
    diamond_size = 16
    draw.polygon(
        [
    ...
    draw.line([(cx, 0), (cx, cy - diamond_size - 8)], fill=ROSE_DIM, width=1)
    draw.line([(cx, cy + diamond_size + 8), (cx, H)], fill=ROSE_DIM, width=1)

    # Top hairline rose accent (full width, faint)
    draw.line([(0, 4), (W, 4)], fill=ROSE_DIM, width=1)
    draw.line([(0, H - 4), (W, H - 4)], fill=ROSE_DIM, width=1)
